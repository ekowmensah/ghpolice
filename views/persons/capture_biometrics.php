...
<div class="biometric-page">
    <!-- Breadcrumb -->
    <nav aria-label="breadcrumb" style="background: transparent; padding: 10px 0;">
        <ol class="breadcrumb" style="background: transparent; padding: 0;">
            <li class="breadcrumb-item"><a href="' . url('/') . '"><i class="fas fa-home"></i> Home</a></li>
            <li class="breadcrumb-item"><a href="' . url('/persons') . '">Persons</a></li>
            <li class="breadcrumb-item"><a href="' . url('/persons/' . $personId) . '">' . htmlspecialchars($personName) . '</a></li>
            <li class="breadcrumb-item active" aria-current="page">Biometrics</li>
        </ol>
    </nav>
    ...
    <div class="biometric-header">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                ...
                <p class="mb-0 mt-2" style="opacity: 0.9;">Person: <strong>' . htmlspecialchars($personName) . '</strong></p>
            </div>
...
foreach ($leftFingers as $finger) {
    ...
    foreach ($biometrics as $bio) {
        ...
        if ($bio['biometric_type'] === 'Fingerprint' && 
            ...
            $captured = true;
            $capturedImage = $bio['file_path'] ?? null;
            break;
        }
    }
    ...
    if ($captured && $capturedImage) {
        ...
        $content .= '<div class="finger-card ' . $capturedClass . '" onclick="captureFingerprint(\'' . $finger['name'] . '\')">
                        ...
                        <div class="capture-badge"><i class="fas fa-check"></i></div>
                    </div>';
    } else {
        // Show icon for uncaptured
        $content .= '<div class="finger-card ' . $capturedClass . '" onclick="captureFingerprint(\'' . $finger['name'] . '\')">
    ...
    $("#captureForm").submit(function(e) {
        ...
        fetch(croppedData)
            ...
            .then(blob => {
                ...
                $.ajax({
                    url: "' . url('/persons/' . $personId . '/biometrics') . '",
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
...
function exportFingerprintSheet() {
    const personName = "' . htmlspecialchars($personName) . '";
    ...
    ctx.font = "14px Arial";
    ctx.textAlign = "left";
    ctx.fillText("Person: " + personName, 50, 110);
    ctx.fillText("Date: " + new Date().toLocaleDateString(), 50, 135);
    ctx.fillText("Person ID: " + personId, 450, 110);
    ...
    function downloadCanvas() {
        ...
        canvas.toBlob(function(blob) {
            ...
            a.href = url;
            a.download = "Fingerprints_" + personName.replace(/\s+/g, "_") + "_" + Date.now() + ".png";
